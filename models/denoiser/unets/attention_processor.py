...
class JointAttnROPEProcessor2_0:
    """Attention processor used typically in processing the SD3-like self-attention projections."""
        ...
        **kwargs,
        ...
        if image_rotary_emb is not None:
            query = torch.cat(
                [
                    apply_rotary_emb(
                        query[:, :, : residual.shape[1]], image_rotary_emb
                    ),
                    query[:, :, residual.shape[1] :],
                ],
                dim=2,
            )
            key = torch.cat(
                [
                    apply_rotary_emb(key[:, :, : residual.shape[1]], image_rotary_emb),
                    key[:, :, residual.shape[1] :],
                ],
                dim=2,
            )
...
class AttnRopeProcessor2_0:
    ...
    def __call__(
        ...
        attention_mask: Optional[torch.Tensor] = None,
        temb: Optional[torch.Tensor] = None,
        frame_rotary_emb: Optional[torch.Tensor] = None,
        *args,
        **kwargs,
    ) -> torch.Tensor:
        ...
        # Apply RoPE if needed
        if frame_rotary_emb is not None:
            # (batch_size, num_heads, seq_len, head_dim)
            query = apply_rope(query, frame_rotary_emb)
            if not attn.is_cross_attention:
                key = apply_rope(key, frame_rotary_emb)
            ...
            if not skip:
                if mask is not None:
                    ...
                    for i in range(current_num_images):
                        ...
                        _current_ip_hidden_states = torch.bmm(
