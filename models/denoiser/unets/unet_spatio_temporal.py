...
from models.denoiser.unets.unets_blocks import (
...
class UNetSpatioTemporalRopeConditionModel(
    ...
    def forward(
        ...
        processed_encoder_states = {}
        for res, states in encoder_hidden_states.items():
        # [batch, channels, frames, height, width] -> [batch * frames, channels, height, width]
            states = states.permute(0, 2, 1, 3, 4).flatten(0, 1)
            processed_encoder_states[res] = states
        ...
        current_resolution = (16, 16)
        down_block_res_samples = (sample,)
        for downsample_block in self.down_blocks:
            if (hasattr(downsample_block, "has_cross_attention") and 
                downsample_block.has_cross_attention and 
                current_resolution in processed_encoder_states):
                
                current_encoder_states = processed_encoder_states[current_resolution]
        ...
        # 4. mid
        last_encoder_states = processed_encoder_states[min(processed_encoder_states.keys())]
        sample = self.mid_block(
            hidden_states=sample,
            temb=emb,
            encoder_hidden_states=last_encoder_states,
            image_only_indicator=image_only_indicator,
            position_ids=position_ids,
        )
        ...
        for i, upsample_block in enumerate(self.up_blocks):
            ...
            if (hasattr(upsample_block, "has_cross_attention") and 
                upsample_block.has_cross_attention and 
                current_resolution in processed_encoder_states):
                current_encoder_states = processed_encoder_states[current_resolution]
