...
class Denoiser(nn.Module):
    def __init__(
        ...
        self.time_reduction_conv = nn.Conv3d(
    ...
    def forward(
        ...
        reference_frames: torch.FloatTensor,
        conditioning_frames: torch.FloatTensor,
        index_distances: torch.Tensor,
        his_csi: torch.FloatTensor,
        ...
        x1 = torch.cat([input_latents, reference_frames, conditioning_frames], dim=2)
        distance = torch.log(index_distances).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
        sample = x1 + distance
        sample = sample.permute(0, 2, 1, 3, 4)
        context_features = self.contextnet(his_csi)
        ...
        output = self.time_reduction_conv(output)
        return output
